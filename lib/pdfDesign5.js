...
//    The title auto-shrinks (and, as a last resort, truncates with an
//    ellipsis) to guarantee a single line inside the fixed-height pill
//    — see drawChapterHeader/fitChapterText below.
...
const CHAPTER_HEADER_MIN_SIZE = 10;
...
// final { text, size, width } to draw with.
//
// This exists because PDFKit's doc.text() with a `width` option always
// routes through its LineWrapper — `lineBreak: false` only suppresses
// splitting on explicit "\n" characters, it does NOT prevent wrapping
// when the rendered string is wider than the given `width`. The
// previous version of this function clamped `w` down to `maxTextW`
// whenever the real text was wider, then passed that too-small `w` as
// the text() width — which silently wrapped long chapter titles onto a
// second line and blew out the fixed-height pill. The fix: never pass a
// width smaller than what the (possibly shrunk/truncated) text actually
// needs, so a wrap can never be triggered.
...
function drawChapterHeader(doc, colors, chapterText, x, y, width) {
  ...
  if (arabic) {
    ...
    doc.font(FONT_ARABIC_BOLD).fontSize(fitted.size).fillColor(colors.orange);
  } else {
    ...
    doc.font(FONT_HEADING).fontSize(fitted.size).fillColor(colors.orange);
  }

  const textY = y + (CHAPTER_HEADER_H - fitted.size) / 2 - 1;
  // fitted.width is always >= the actual rendered width of fitted.text,
  // so this can never trigger PDFKit's wrap-to-width behavior.
