...
// Question text now renders as its own pale-yellow highlight chip —
// the reference sheet's "Enumerate management functions" style callout
// — instead of plain colored text, per the user's request. The chip
// spans the full available width (matching the reference's full-width
// yellow banners) and wraps its own text internally, so measurement
// returns fixed wrapped lines (not doc.heightOfString) the same way the
// 'highlight' answer-paragraph blocks already do above.
...
function drawQuestion(doc, measured, x, y, width) {
  ...
  doc.save();
  doc.roundedRect(x, y, width, h, 8).fill(YELLOW_BG);
  doc.strokeColor(YELLOW_BORDER).lineWidth(1);
  doc.roundedRect(x, y, width, h, 8).stroke();
  doc.restore();
...
function drawFooter(doc, pageNum, pageWidth, pageHeight, bookName, footerBrand) {
  ...
  // page edges by the same margin as the content (not an edge-to-edge
  // rectangle), with the book name left, brand right, and a solid navy
  // page-number circle centered on the bar, slightly overlapping its top
  // edge like a badge — this is a pixel-level match for the "Ultra-short
  // [n] Victorio Team" reference footer.
