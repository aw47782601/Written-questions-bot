...
// (title box, badges, category labels), a soft yellow highlight chip, and
// a green checkmark. These stay fixed regardless of meta.colorKey — they're
// the reference sheet's own fixed-identity accents (same convention as
// design_7's RED/GREEN/YELLOW), not the single re-tintable hue.
//
// The teal accent (section/chapter banners, comparison-table header,
// footer page-number circle, bullet markers, highlight-chip text) IS the
// one hue this design is meant to be re-tinted by (same as design_7's
// "purple" accent) — derived per-call from meta.colorKey via
// getColorTheme() and threaded through as the `colors` param on
// drawChapterHeader/drawComparisonTable/drawFooter/drawAnswer/drawDivider
// below (see buildAccentColors). It defaults to teal only because
// DEFAULT_PDF_COLOR resolves to a teal-ish base by default in this design;
...
// getColorTheme() result — mirrors design_7's buildAccentColors() so both
// designs re-tint from meta.colorKey the same way.
...
function drawQuestion(doc, measured, x, y, width) {
  ...
  if (measured.arabic) {
    ...
    doc.restore();
    drawArabicLines(doc, measured.lines, textX, textY, innerWidth, FONT_ARABIC_BOLD, QUESTION_SIZE, LINE_HEIGHT_Q, '#000000');
  } else {
...
// The "# " callout-heading block (a Gemini-flagged highlight line inside
// an answer) has its own small per-word wrap/draw pair, separate from
// tokenizeHighlighted/wrapTokens above — same Greek/symbol fallback
// reasoning applies here too.
function highlightChipTokenFont(greek) {
  return greek ? FONT_GREEK_BOLD : FONT_HEADING;
}

function wrapHighlightChipWords(doc, words, size, maxWidth) {
  doc.font(FONT_HEADING).fontSize(size);
  const spaceWidth = doc.widthOfString(' ');
  const lines = [];
  let current = [];
  ...
  words.forEach((word) => {
    const greek = hasGreek(word);
    doc.font(highlightChipTokenFont(greek)).fontSize(size);
...
function drawHighlightChipLine(doc, line, x, y, size, color) {
  let cx = x;
  line.forEach((tok) => {
    doc.font(highlightChipTokenFont(tok.greek)).fontSize(size).fillColor(color);
    const spaceWidth = doc.widthOfString(' ');
    doc.text(tok.word, cx, y, { lineBreak: false });
    cx += doc.widthOfString(tok.word) + spaceWidth;
  });
}
...
function measureAnswer(doc, answer, width) {
  ...
  const blocks = paragraphs.map((raw) => {
    ...
    if (kind === 'highlight') {
      ...
      const lines = wrapHighlightChipWords(doc, words, LABEL_SIZE + 1, width - HIGHLIGHT_CHIP_PAD_X * 2);
...
function drawAnswer(doc, measured, x, y, width, colors) {
  let cy = y;
  measured.blocks.forEach((b) => {
    if (b.kind === 'highlight') {
      ...
      b.lines.forEach((line, i) => {
        drawHighlightChipLine(
          doc,
          line,
          x + HIGHLIGHT_CHIP_PAD_X,
          cy + HIGHLIGHT_CHIP_PAD_Y + i * (LABEL_SIZE + 6),
          LABEL_SIZE + 1,
          colors.dark
        );
      });
...
function drawComparisonTable(doc, measured, x, y, width, colors) {
  ...
  // Solid colorKey-tinted header band (clipped to the table's rounded top
  // corners).
  doc.save();
...
function drawAnswerBody(doc, body, x, y, width, colors) {
  ...
  if (body.kind === 'table') {
    if (body.intro) {
      drawAnswer(doc, body.intro, x, cy, width, colors);
      cy += body.intro.height + PARA_GAP;
    }
    drawComparisonTable(doc, body.measured, x, cy, width, colors);
  } else {
    drawAnswer(doc, body.measured, x, y, width, colors);
  }
}
...
function drawChapterHeader(doc, measured, pageContentX, y, pageContentWidth, colors) {
  ...
  doc.roundedRect(capsuleX, y, capsuleW, capsuleH, r).fill(colors.bright);
...
// drive this design's teal-by-default accent trio (see
// buildAccentColors above): pass a different preset/hex and the chapter
// banner, comparison-table header, answer bullets/highlight-chip text,
// and "ANSWER" label all re-tint together. title (if given) draws the
// page-1 hero box; bookName/footerBrand feed the footer row.
function generateAnswersPdf(results, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      // Accent trio derived from meta.colorKey — see buildAccentColors
      // above. Every chapter banner, table header, bullet marker, and
      // highlight-chip/label below is painted from this one object
      // instead of a fixed hex, so picking a different colorKey
      ...
      results.forEach((r, idx) => {
        ...
        function drawFullItem() {
          ...
          drawDivider(doc, textColX, curY, textColWidth, colors);
          curY += 12;

          doc.font(FONT_BOLD).fontSize(LABEL_SIZE).fillColor(colors.dark);
        ...
        if (itemHeight <= availableOnPage || aBody.kind !== 'text') {
          ...
          if (firstBlocks.length === 0 || restBlocks.length === 0) {
            ...
            drawAnswer(doc, { blocks: firstBlocks }, answerX, curY, fullTextWidth, colors);
            ...
            drawAnswer(doc, { blocks: restBlocks }, answerX, curY2, fullTextWidth, colors);
