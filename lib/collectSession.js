...
async function getSession(userId) {
  ...
  // this bot may still be sitting in storage in the OLD shape
  // ({ questions: [...] }, from back when extraction ran per-message).
  // Coerce anything old/malformed into the current { rawTexts, lineCount }
  // shape instead of throwing, so an in-flight /text session survives a
  // deploy instead of crashing the next message.
  if (!Array.isArray(session.rawTexts)) {
    session.rawTexts = [];
    session.lineCount = 0;
  }
  if (typeof session.lineCount !== 'number') {
    session.lineCount = 0;
  }
  return session;
}
