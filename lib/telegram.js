...
// Uploads a Buffer (e.g. a rendered book page whose caption/image-embedding
// matched the student's question — see lib/books.js ingestPageImages and
// lib/batchAnswer.js) as a Telegram photo. Same FormData/Blob pattern as
// sendDocument above.
async function sendPhoto(chatId, buffer, extra = {}) {
  const form = new FormData();
  form.append('chat_id', String(chatId));
  if (extra.caption) form.append('caption', extra.caption);
  if (extra.parse_mode) form.append('parse_mode', extra.parse_mode);
  form.append('photo', new Blob([buffer], { type: 'image/jpeg' }), 'page.jpg');

  const res = await fetch(`${API_BASE}/sendPhoto`, { method: 'POST', body: form });
  const data = await res.json();
  if (!data.ok) {
    console.error('Telegram API error on sendPhoto:', data);
  }
  return data;
}
