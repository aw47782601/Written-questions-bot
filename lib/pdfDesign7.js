...
function drawQuestion(doc, measured, x, y, width) {
  ...
  if (measured.arabic) {
    // Right-aligned per line, black, no underline.
    doc.font(FONT_ARABIC_BOLD).fontSize(QUESTION_SIZE).fillColor('#000000');
    ...
    // still work inline), every word black, no underline.
    measured.lines.forEach((line, i) => {
      let cx = textX;
      const ly = textY + i * LINE_HEIGHT_Q;
      line.forEach((tok) => {
        doc.font(questionTokenFont(tok)).fontSize(QUESTION_SIZE).fillColor('#000000');
        const spaceWidth = doc.widthOfString(' ');
        const tokW = doc.widthOfString(tok.shaped);
        doc.text(tok.shaped, cx, ly, { lineBreak: false });
        cx += tokW + spaceWidth;
