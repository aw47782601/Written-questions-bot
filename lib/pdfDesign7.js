...
// red accent for badges/category labels/underlined key terms, a soft
// yellow highlight chip, a green checkmark, a salmon chevron-notch
// accent, and a plain page number on a light-gray wavy footer band.
// These stay fixed regardless of meta.colorKey — they're the reference
// sheet's own fixed-identity accents (key-term red, checkmark green,
// highlighter yellow), not the single re-tintable hue.
//
// The "purple" accent (chapter-cover circle, chevron banners, table
// header, footer badge) is NOT fixed, though — it's the one hue this
// design is meant to be re-tinted by (same as design_1–design_5's single
// accent), derived per-call from meta.colorKey via getColorTheme() and
// threaded through as the `colors` param on drawHero/drawChapterHeader/
// drawBadge/drawFooter/drawComparisonTable below (see buildAccentColors).
// It defaults to purple only because DEFAULT_PDF_COLOR resolves to a
// purple-ish base — pick a different colorKey and every one of those
// elements re-tints together.
...
// Derives this design's { bright, dark, pastel } accent trio from a
// getColorTheme() result — bright/dark are the theme's own base/darker
// shades (solid fills, text-on-white), pastel is its pale "divider"
// tint (banner/hero-circle backgrounds) — so every accent element below
// re-tints together from one colorKey instead of being individually
// hand-picked per design the way the old fixed purple hexes were.
function buildAccentColors(theme) {
  return { bright: theme.orange, dark: theme.orangeDark, pastel: theme.divider };
}

// Footer brand text — the reference sheet's own two-sided footer label
// ("Victoreo 67" left / "Ultra team" right), flanking a solid
// (colorKey-tinted) page-number roundel centered on the wavy band.
const DEFAULT_FOOTER_LEFT = 'Victoreo 67';
const DEFAULT_FOOTER_RIGHT = 'Ultra team';

// Fixed brand shown as the page-1 hero title whenever a batch spans more
// than one chapter (see heroTitle in generateAnswersPdf below) — the
// same convention design_6 already uses.
const DEFAULT_HERO_TITLE = 'Victorio 67';
...
// Gap between the bottom of a question's full-width yellow bar and the
// first line of its answer — replaces the old divider-rule + "ANSWER"
// label spacing now that both have been dropped to match the reference
// sheet's plain run-on layout.
const QUESTION_ANSWER_GAP = 10;
...
// Question text renders inside a solid pale-yellow bar that spans the
// FULL available row width (not a tight-fit chip hugging the text) —
// matching TEMPLATE_2.pdf's numbered "No role of using enhancement of
// elimination"-style full-width highlight bars. The question's own
// number ("1.", "2." ...) is drawn separately, outside this bar, in the
// badge column to its left (see drawBadge).
const QUESTION_CHIP_PAD_X = 14;
...
function drawQuestion(doc, measured, x, y, width) {
  ...
  // Solid pale-yellow bar spanning the FULL row width — one rect for
  // the whole question block, not per-line — matching the reference
  // sheet's full-width numbered highlight bars.
  doc.save();
  doc.fillColor(YELLOW_BG);
  doc.rect(x, y, width, measured.height).fill();
  doc.restore();

  if (measured.arabic) {
    // Right-aligned per line, red, with a manually-drawn underline
    // spanning the whole phrase (pdfkit's built-in {underline:true}
    // option can't be combined with {lineBreak:false}, which is needed
    // here for exact positioning, so the rule is drawn by hand instead).
    doc.font(FONT_ARABIC_BOLD).fontSize(QUESTION_SIZE).fillColor(RED);
    measured.lines.forEach((line, i) => {
      const shaped = shapeArabicLine(line);
      const w = doc.widthOfString(shaped);
      const lx = textX + Math.max(0, innerWidth - w);
      const ly = textY + i * LINE_HEIGHT_Q;
      doc.text(shaped, lx, ly, { lineBreak: false });
      doc.save();
      doc.strokeColor(RED).lineWidth(1);
      doc.moveTo(lx, ly + QUESTION_SIZE + 1).lineTo(lx + w, ly + QUESTION_SIZE + 1).stroke();
      doc.restore();
    });
  } else {
    // Text itself, token by token (so Greek/Arabic-run fallback fonts
    // still work inline), every word red with a hand-drawn underline
    // beneath it — matching the reference sheet's red underlined
    // question phrasing inside the yellow bar.
    measured.lines.forEach((line, i) => {
      let cx = textX;
      const ly = textY + i * LINE_HEIGHT_Q;
      line.forEach((tok) => {
        doc.font(questionTokenFont(tok)).fontSize(QUESTION_SIZE).fillColor(RED);
        const spaceWidth = doc.widthOfString(' ');
        const tokW = doc.widthOfString(tok.shaped);
        doc.text(tok.shaped, cx, ly, { lineBreak: false });
        doc.save();
        doc.strokeColor(RED).lineWidth(1);
        doc.moveTo(cx, ly + QUESTION_SIZE + 1).lineTo(cx + tokW, ly + QUESTION_SIZE + 1).stroke();
        doc.restore();
        cx += tokW + spaceWidth;
      });
    });
...
function drawComparisonTable(doc, measured, x, y, width, colors) {
  ...
  doc.rect(x, y, width, headerHeight).fill(colors.bright);
  let cx = x;
  ...
  bodyRows.forEach((row) => {
    ...
    row.cells.forEach((cell, i) => {
      doc.rect(rx, ry, colWidths[i], row.height).fill(i === 0 ? colors.pastel : CARD_BG);
...
function drawAnswerBody(doc, body, x, y, width, colors) {
  let cy = y;
  if (body.kind === 'table') {
    if (body.intro) {
    ...
    drawComparisonTable(doc, body.measured, x, cy, width, colors);
  } else {
    drawAnswer(doc, body.measured, x, y, width);
  }
}

// Divider — a thin colorKey-accented rule with a small solid square
// tucked at its left end, echoing the reference sheet's section rules.
// (Currently unused by the main per-question loop, kept for parity with
// design_6's helper set.)
function drawDivider(doc, x, y, width, colors) {
  doc.save();
  doc.fillColor(colors.bright);
...
function drawChapterHeader(doc, measured, pageContentX, y, pageContentWidth, colors) {
  ...
  // Light pastel ribbon bar, full content width, with a crisp dark
  // outline in the same colorKey-derived hue — matching the reference
  // sheet's bordered "COMPLETE" / "WRITTEN QS" / "COMPARE QS" banners
  // (not a flat borderless pill).
  doc.roundedRect(barX, y, barW, barH, radius).fill(colors.pastel);
  doc.save();
  doc.strokeColor(colors.dark).lineWidth(1.4);
  doc.roundedRect(barX, y, barW, barH, radius).stroke();
  doc.restore();
  ...
  doc.moveTo(barX + radius, y)
    ...
    .closePath()
    .fill(CHEVRON_NOTCH);
  doc.strokeColor(colors.dark).lineWidth(1.2);
  ...
  doc.font(arabic ? FONT_ARABIC_BOLD : FONT_HEADING).fontSize(fitted.size).fillColor(colors.dark);
...
// very bottom, with bold colorKey-tinted brand labels on the left
// ("Victoreo 67") and right ("Ultra team"), flanking a solid
// colorKey-tinted page-number roundel centered on the band — a direct
// match for the reference sheet's own footer row (meta.footerBrand is
// not wired up here since the reference uses this fixed two-sided
// label, not a single brand string).
const WAVE_AMPLITUDE = 10;
const FOOTER_BADGE_R = 14;

function drawFooter(doc, pageNum, pageWidth, pageHeight, colors) {
  ...
  const rowY = pageHeight - FOOTER_H / 2 - 7;

  doc.font(FONT_HEADING).fontSize(13).fillColor(colors.bright);
  doc.text(DEFAULT_FOOTER_LEFT, PAGE_MARGIN, rowY, { lineBreak: false });

  doc.font(FONT_HEADING).fontSize(13).fillColor(colors.bright);
  const rightW = doc.widthOfString(DEFAULT_FOOTER_RIGHT);
  doc.text(DEFAULT_FOOTER_RIGHT, pageWidth - PAGE_MARGIN - rightW, rowY, { lineBreak: false });

  // Solid colorKey-tinted roundel, centered on the band, holding the
  // page number in white — the reference sheet's page-number badge (not
  // a plain number floating on the band).
  const cx = pageWidth / 2;
  const cy = pageHeight - FOOTER_H / 2 - 2;
  doc.save();
  doc.fillColor(colors.dark);
  doc.circle(cx, cy, FOOTER_BADGE_R).fill();
  doc.restore();

  const pageNumText = String(pageNum);
  doc.font(FONT_HEADING).fontSize(14).fillColor('#FFFFFF');
  const pw = doc.widthOfString(pageNumText);
  doc.text(pageNumText, cx - pw / 2, cy - 7, { lineBreak: false });
}

// Question number — plain bold colorKey-tinted "1." / "2." ... sitting
// to the LEFT of the question's own full-width yellow bar (not a
// circular badge on top of it), matching the reference sheet's
// numbered-list style ("1. No role of using enhancement of elimination").
const BADGE_SIZE = 24;

function drawBadge(doc, idx, x, y, colors) {
  const numText = `${idx + 1}.`;
  doc.font(FONT_HEADING).fontSize(QUESTION_SIZE - 2).fillColor(colors.dark);
  doc.text(numText, x, y, { width: BADGE_SIZE, lineBreak: false });
}

// Small decorative "school supplies" doodle drawn with basic vector
// shapes — an original stand-in for the reference sheet's stationery
// clip-art icon (that exact raster icon isn't something we can
// license/ship, so this hand-drawn approximation reuses the same motif:
// a stack of books topped by a bubbling test tube, plus a small loose
// dot) sitting on the purple hero circle.
const DOODLE_INK = '#2A1A40';
...
function drawDoodleIcon(doc, cx, cy) {
  ...
  doc.save();
  doc.lineWidth(1.4).strokeColor(DOODLE_INK);

  // Bottom book — salmon/pink, sitting lowest in the stack.
  doc.roundedRect(x, topY + bookH * 2, w, bookH, 3).fillAndStroke('#F5A9A4', DOODLE_INK);

  // Middle book — white, slightly narrower, with a spine crease.
  const midW = w - 8;
  const midX = x + (w - midW) / 2;
  doc.roundedRect(midX, topY + bookH, midW, bookH, 2).fillAndStroke('#FFFFFF', DOODLE_INK);
  doc.moveTo(midX + midW / 2, topY + bookH + 2).lineTo(midX + midW / 2, topY + bookH * 2 - 2).stroke();

  // Top book — mustard yellow, narrower still, with its own spine
  // crease — the "open" book the test tube stands on.
  const topW = w - 20;
  const topX = x + (w - topW) / 2;
  doc.roundedRect(topX, topY, topW, bookH, 2).fillAndStroke('#F5C34A', DOODLE_INK);
  doc.moveTo(topX + topW / 2, topY + 2).lineTo(topX + topW / 2, topY + bookH - 2).stroke();

  // Test tube standing upright on the top book, angled slightly, with a
  // rounded bottom, a bit of "liquid" fill, and two small bubbles.
  const tubeW = 11;
  const tubeH = 30;
  const tubeX = cx - tubeW / 2 + 4;
  const tubeY = topY - tubeH + 6;
  doc.save();
  doc.rotate(6, { origin: [tubeX + tubeW / 2, tubeY + tubeH] });
  doc.roundedRect(tubeX, tubeY, tubeW, tubeH, tubeW / 2).fillAndStroke('#EAF2FB', DOODLE_INK);
  doc.save();
  doc.roundedRect(tubeX + 1.5, tubeY + tubeH * 0.45, tubeW - 3, tubeH * 0.5, (tubeW - 3) / 2).fill('#8FD3C7');
  doc.restore();
  doc.fillColor(DOODLE_INK);
  doc.circle(tubeX + tubeW / 2 + 2, tubeY + 9, 1.3).fill();
  doc.circle(tubeX + tubeW / 2 - 2, tubeY + 15, 1).fill();
  doc.restore();

  // Small loose accent dot, top-right of the stack.
  doc.fillColor('#F2A93B');
  doc.circle(x + w + 6, topY - 4, 3).fill();

  doc.restore();
}

// Chapter-cover header — drawn once, at the top of page 1, when
// meta.title is given: a big soft colorKey-tinted circle bleeding off
// the top-left corner with a small doodle icon on it, and the title set
// as an oversized drop-cap first letter followed by the rest of the
// title in bold colorKey-tinted ink — a direct match for the reference
// sheet's cover-page treatment ("Chapter name" with a giant "C").
function drawHero(doc, title, pageWidth, colors) {
  const circleCx = 78;
  const circleCy = 74;
  const circleR = HERO_CIRCLE_D / 2;

  doc.save();
  doc.fillColor(colors.pastel);
  ...
  doc.font(FONT_HEADING).fontSize(HERO_DROPCAP_SIZE).fillColor(colors.dark);
  ...
  doc.font(FONT_HEADING).fontSize(restSize).fillColor(colors.dark);
...
// meta: { title, bookName, footerBrand, colorKey } — colorKey now DOES
// drive this design's purple-by-default accent trio (see
// buildAccentColors above): pass a different preset/hex and every
// chapter banner, hero circle, table header, badge number, and footer
// brand/roundel re-tints together. title (if given) draws the page-1
// hero box; bookName/footerBrand feed the footer row.
function generateAnswersPdf(results, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      // Purple-by-default accent trio, derived from meta.colorKey — see
      // buildAccentColors above. Every chapter banner, hero circle,
      // table header, badge number, and footer brand/roundel below is
      // painted from this one object instead of a fixed hex, so picking
      // a different colorKey re-tints the whole design together.
      const colors = buildAccentColors(getColorTheme(meta.colorKey || DEFAULT_PDF_COLOR));
      ...
      // title instead of the fixed "Victorio 67" brand — there's no
      // point repeating the chapter name in its own banner right below
      // a hero that just says the brand name. With more than one
      // chapter, the hero always reads the fixed brand name (same
      // convention as design_6) — the caller's meta.title text itself
      // is only used as the on/off switch for drawing the hero at all,
      // never shown verbatim.
      const uniqueChapters = Array.from(new Set(results.map((r) => r.chapter).filter(Boolean)));
      const heroTitle = uniqueChapters.length === 1 ? uniqueChapters[0] : DEFAULT_HERO_TITLE;
      // With a single chapter, its name is already shown in the page-1
      // hero above — the per-chapter banner pill would just repeat it,
      // so skip drawing that banner entirely in this case. With more
      // than one chapter (including the very first one), every chapter
      // change still gets its own banner — nothing is skipped there.
      ...
      function startPage() {
        ...
        if (pageNum === 1 && meta.title && !heroDrawnOnPage1) {
          // Design 7's first-page hero shows heroTitle (the fixed brand
          // name for a multi-chapter batch, or the chapter's own name
          // for a single-chapter batch) as the drop-cap cover title.
          drawHero(doc, heroTitle, pageWidth, colors);
      ...
      results.forEach((r, idx) => {
        if (r.chapter && r.chapter !== lastChapter) {
          if (singleChapterMode) {
            ...
            drawChapterHeader(doc, chMeasured, contentX, y, contentWidth, colors);
        ...
        const qMeasured = measureQuestion(doc, r.question, textColWidth);
        const aBody = measureAnswerBody(doc, r, fullTextWidth);
        const pageNoteText = r.page ? `(page ${r.page})` : null;

        // No "ANSWER" label or divider rule between the question bar
        // and its answer — the reference sheet's numbered items run
        // straight from the yellow question bar into the checkmark/
        // bullet answer lines below it, with just a small gap.
        const itemHeight =
          qMeasured.height +
          QUESTION_ANSWER_GAP +
          aBody.height +
        ...
        function drawFullItem() {
          ...
          drawBadge(doc, idx, contentX, itemY + QUESTION_CHIP_PAD_Y, colors);

          let curY = itemY;
          drawQuestion(doc, qMeasured, textColX, curY, textColWidth);
          curY += qMeasured.height + QUESTION_ANSWER_GAP;

          drawAnswerBody(doc, aBody, answerX, curY, fullTextWidth, colors);
          curY += aBody.height;
        ...
        if (itemHeight <= availableOnPage || aBody.kind !== 'text') {
          ...
          const headerHeight = qMeasured.height + QUESTION_ANSWER_GAP;
          ...
          if (firstBlocks.length === 0 || restBlocks.length === 0) {
            ...
            drawFooter(doc, pageNum, pageWidth, pageHeight, colors);
            doc.addPage();
            pageNum += 1;
            startPage();
