...
//
// The single accent hue (chapter-cover circle, chevron banners, table
...
// It follows whatever colorKey the caller passes (falling back to
// pdfColors.DEFAULT_PDF_COLOR, currently coral, only when none is given)
// — pick a different colorKey and every one of those elements re-tints
// together.
...
// drive this design's accent trio (see buildAccentColors above): pass a
// different preset/hex and every chapter banner, hero circle, table
// header, badge number, and footer brand/roundel re-tints together.
// title (if given) draws the page-1 hero box; bookName/footerBrand feed
// the footer row.
function generateAnswersPdf(results, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      // Accent trio derived from meta.colorKey — see buildAccentColors
      // above. Every chapter banner, hero circle, table header, badge
      // number, and footer brand/roundel below is painted from this one
      // object instead of a fixed hex, so picking a different colorKey
      // re-tints the whole design together.
