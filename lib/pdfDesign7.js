...
// Chapter/section ribbon banner — a lighter lavender fill than TEAL
// (which stays reserved for dividers/table headers/etc.), plus a dark
// outline stroke, matching TEMPLATE_2.pdf's "COMPLETE" / "WRITTEN QS" /
// "COMPARE QS" banners (a light-purple bar with a crisp dark border,
// not a flat borderless purple pill).
const CHAPTER_BAR_BG = '#CBB0F0';
const CHAPTER_BAR_BORDER = '#241536';

// Footer brand text — the reference sheet's own two-sided footer label
// ("Victoreo 67" left / "Ultra team" right), flanking a solid purple
// page-number roundel centered on the wavy band.
const DEFAULT_FOOTER_LEFT = 'Victoreo 67';
const DEFAULT_FOOTER_RIGHT = 'Ultra team';
const FOOTER_BADGE_COLOR = '#5B2A86';
...
function drawChapterHeader(doc, measured, pageContentX, y, pageContentWidth) {
  ...
  // Light-lavender ribbon bar, full content width, with a crisp dark
  // outline — matching the reference sheet's bordered "COMPLETE" /
  // "WRITTEN QS" / "COMPARE QS" banners (not a flat borderless purple
  // pill).
  doc.roundedRect(barX, y, barW, barH, radius).fill(CHAPTER_BAR_BG);
  doc.save();
  doc.strokeColor(CHAPTER_BAR_BORDER).lineWidth(1.4);
  ...
  doc.moveTo(barX + radius, y)
    ...
    .closePath()
    .fill(CHEVRON_NOTCH);
  doc.strokeColor(CHAPTER_BAR_BORDER).lineWidth(1.2);
...
// very bottom, with bold purple brand labels on the left ("Victoreo
// 67") and right ("Ultra team"), flanking a solid purple page-number
// roundel centered on the band — a direct match for the reference
// sheet's own footer row (meta.footerBrand is not wired up here since
// the reference uses this fixed two-sided label, not a single brand
// string).
...
function drawFooter(doc, pageNum, pageWidth, pageHeight) {
  ...
  doc.font(FONT_HEADING).fontSize(13).fillColor(TEAL);
  ...
  // Solid purple roundel, centered on the band, holding the page number
  // in white — the reference sheet's page-number badge (not a plain
  // number floating on the band).
  const cx = pageWidth / 2;
  const cy = pageHeight - FOOTER_H / 2 - 2;
  doc.save();
  doc.fillColor(FOOTER_BADGE_COLOR);
...
// Question number — plain bold indigo "1." / "2." ... sitting to the
// LEFT of the question's own full-width yellow bar (not a circular
// badge on top of it), matching the reference sheet's numbered-list
// style ("1. No role of using enhancement of elimination").
const BADGE_SIZE = 24;

function drawBadge(doc, idx, x, y) {
  const numText = `${idx + 1}.`;
  doc.font(FONT_HEADING).fontSize(QUESTION_SIZE - 2).fillColor(HERO_TITLE_COLOR);
    ...
    try {
      ...
      results.forEach((r, idx) => {
        ...
        function drawFullItem() {
          ...
          drawBadge(doc, idx, contentX, itemY + QUESTION_CHIP_PAD_Y);
