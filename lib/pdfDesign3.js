...
// Spectral has almost no Greek glyph coverage (confirmed via font cmap
// inspection), so alpha/beta/gamma-style symbols in a science/math answer
// silently drew as blank/missing-glyph boxes. Lato covers essentially the
// whole Greek alphabet, so it's registered purely as a per-word fallback
// — see tokenFont() below — without changing this design's Spectral look
// for everything else.
...
function drawChapterHeader(doc, colors, chapterText, x, y, width) {
  ...
  const textY = y + (CHAPTER_HEADER_H - CHAPTER_HEADER_SIZE) / 2 + 2;
  if (isMostlyArabic(chapterText)) {
    const shaped = shapeArabicLine(chapterText);
    doc.font(FONT_ARABIC_BOLD).fontSize(CHAPTER_HEADER_SIZE).fillColor(colors.orangeDark);
    const w = doc.widthOfString(shaped);
    doc.rect(cx - w / 2 - 8, textY - 2, w + 16, CHAPTER_HEADER_SIZE + 4).fill(PAGE_BG);
    doc.fillColor(colors.orangeDark);
    doc.text(shaped, cx - w / 2, textY, { lineBreak: false });
  } else {
    doc.font(FONT_HEADING).fontSize(CHAPTER_HEADER_SIZE).fillColor(colors.orangeDark);
    const upper = chapterText.toUpperCase();
    const w = doc.widthOfString(upper, { characterSpacing: 1 });
    doc.rect(cx - w / 2 - 8, textY - 2, w + 16, CHAPTER_HEADER_SIZE + 4).fill(PAGE_BG);
    doc.fillColor(colors.orangeDark);
    doc.text(upper, cx - w / 2, textY, { lineBreak: false, characterSpacing: 1 });
  }
}
