    ...
    try {
      ...
      results.forEach((r, idx) => {
        ...
        function drawFullCard() {
          ...
          const cardY = y;
          // Whiter-than-page card in a thin+hairline DOUBLE-rule frame —
          // a book-plate border — instead of design_1's rounded cream
          // card or design_2's flat white card with a colored left
          // accent bar.
          ...
          // classical book's exercise numbering, replacing both
          // design_1's circle badge and design_2's square badge.
          doc.save();
          ...
          // ANSWER label — small caps, letter-spaced, accent-dark,
          // flanked by tiny flourish marks instead of a plain flat label.
        ...
        if (cardHeight <= availableOnPage || aBody.kind !== 'text') {
          ...
          if (firstBlocks.length === 0 || restBlocks.length === 0) {
            ...
            const cardY = y;
            doc.rect(contentX, cardY, contentWidth, firstPartHeight).fill(CARD_BG);
            doc.save();
            doc.strokeColor(colors.orange).lineWidth(1.1);
            doc.rect(contentX, cardY, contentWidth, firstPartHeight).stroke();
            doc.strokeColor(NEUTRAL_BORDER).lineWidth(0.6);
            doc.rect(contentX + 3, cardY + 3, contentWidth - 6, firstPartHeight - 6).stroke();
            doc.restore();
            ...
            drawAnswer(doc, colors, { blocks: firstBlocks }, textColX, curY, textColWidth);
            curY += firstAnswerHeight + 4;

            doc.font(FONT_REGULAR).fontSize(8.5).fillColor(NEUTRAL_GRAY);
            doc.text(`▾ Q${idx + 1} continues on next page…`, textColX, curY, { width: textColWidth });
            ...
            const cardY2 = y;
            doc.rect(contentX, cardY2, contentWidth, secondPartHeight).fill(CARD_BG);
            doc.save();
            doc.strokeColor(colors.orange).lineWidth(1.1);
            doc.rect(contentX, cardY2, contentWidth, secondPartHeight).stroke();
            doc.strokeColor(NEUTRAL_BORDER).lineWidth(0.6);
            doc.rect(contentX + 3, cardY2 + 3, contentWidth - 6, secondPartHeight - 6).stroke();
            doc.restore();

            let curY2 = cardY2 + CARD_PADDING;
            doc.font(FONT_REGULAR).fontSize(8.5).fillColor(NEUTRAL_GRAY);
            doc.text(`↳ Q${idx + 1} (continued):`, textColX, curY2, { width: textColWidth });
            curY2 += CONTINUATION_NOTE_H;

            drawAnswer(doc, colors, { blocks: restBlocks }, textColX, curY2, textColWidth);
