...
async function embedImagesChunk(images, taskType = 'RETRIEVAL_DOCUMENT', extraKeys = [], collector = null, triedKeysThisRound = new Set(), round = 1) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      const newTried = new Set(triedKeysThisRound).add(key);
      if (newTried.size < poolSize) {
        return embedImagesChunk(images, taskType, extraKeys, collector, newTried, round);
      }
      if (round < MAX_ROUNDS) {
        const suggested = parseRetryDelayMs(errText);
        const waitMs = suggested ?? Math.min(2 ** round * 1000, 30000);
        await sleep(waitMs + 500);
        return embedImagesChunk(images, taskType, extraKeys, collector, new Set(), round + 1);
      }
    }
...
async function embedImages(images, taskType = 'RETRIEVAL_DOCUMENT', extraKeys = [], collector = null) {
  if (images.length <= MAX_EMBED_BATCH) {
    return embedImagesChunk(images, taskType, extraKeys, collector);
  }
  const chunks = [];
  for (let i = 0; i < images.length; i += MAX_EMBED_BATCH) {
    chunks.push(images.slice(i, i + MAX_EMBED_BATCH));
  }
  const results = await Promise.all(chunks.map((chunk) => embedImagesChunk(chunk, taskType, extraKeys, collector)));
  return results.flat();
}

// Auto-captions a rendered page image with the existing vision-capable
// generation model (GEMINI_GENERATION_MODEL etc. already accept image
...
async function captionImage(buffer, mimeType, pageNumber, extraKeys = []) {
  const prompt =
  ...
  // runFallbackSequence/generateJSON build a text-only {parts:[{text}]}
  // request body — image input needs its own request shape, so a direct
  // generateContent call is made here instead. Kept intentionally simple
  // (one key, one model, no retry/fallback sequence): a captioning
  // failure falls back to a generic placeholder caption rather than
  // blocking the whole image-ingestion pass or spending a full
  // key×model retry budget on a non-critical, best-effort step.
  try {
    const key = env.GEMINI_API_KEYS[0];
    ...
    const body = {
      ...
      generationConfig: { maxOutputTokens: 1024, thinkingConfig: getThinkingConfig(env.GEMINI_GENERATION_MODEL) },
    };
    const res = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) throw new Error(`caption generateContent error (${res.status}): ${await res.text()}`);
    const json = await res.json();
    const text = json.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!text) throw new Error('Gemini returned no caption text');
    return text.trim();
  } catch (err) {
    console.error(`captionImage failed for page ${pageNumber}:`, err.message);
    return `(صورة صفحة ${pageNumber} — تعذر توليد وصف تلقائي لها)`;
  }
}
...
module.exports = { embedTexts, embedOne, embedImages, captionImage, generateJSON, formatAttemptLog };
