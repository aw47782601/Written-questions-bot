...
function pickKey(excludeKeys, extraKeys = []) {
  const fullPool = extraKeys.length > 0 ? [...env.GEMINI_API_KEYS, ...extraKeys] : env.GEMINI_API_KEYS;
  const pool = fullPool.filter((k) => !excludeKeys.has(k));
  const from = pool.length > 0 ? pool : fullPool;
...
async function embedTexts(texts, taskType = 'RETRIEVAL_DOCUMENT', extraKeys = [], triedKeysThisRound = new Set(), round = 1) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      ...
      if (newTried.size < poolSize) {
        return embedTexts(texts, taskType, extraKeys, newTried, round);
      }
      ...
      if (round < MAX_ROUNDS) {
        ...
        return embedTexts(texts, taskType, extraKeys, new Set(), round + 1);
      }
    }
...
async function generateJSON(prompt, { maxOutputTokens = 8192, responseSchema = null, extraKeys = [] } = {}, triedKeysThisRound = new Set(), round = 1) {
  const key = pickKey(triedKeysThisRound, extraKeys);
  const poolSize = env.GEMINI_API_KEYS.length + extraKeys.length;
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      ...
      // BUG FIX: this used to compare against env.GEMINI_API_KEYS.length only,
      // ignoring extraKeys (a user's own pooled-in keys) — so once as many
      // keys as the *base* pool size had been tried, it fell through to the
      // sleep/backoff branch even if untried extra keys remained. Now matches
      // embedTexts's (correct) use of poolSize.
      if (newTried.size < poolSize) {
        return generateJSON(prompt, { maxOutputTokens, responseSchema, extraKeys }, newTried, round);
      }
      ...
      if (round < MAX_ROUNDS) {
        ...
        const waitMs = suggested ?? Math.min(2 ** round * 1000, 30000);
        await sleep(waitMs + 500);
        return generateJSON(prompt, { maxOutputTokens, responseSchema, extraKeys }, new Set(), round + 1);
      }
    }
  ...
  if (candidate?.finishReason === 'MAX_TOKENS' && maxOutputTokens < 32768) {
    return generateJSON(prompt, { maxOutputTokens: maxOutputTokens * 2, responseSchema, extraKeys }, triedKeysThisRound, round);
  }
