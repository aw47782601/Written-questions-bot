...
async function captionImage(buffer, mimeType, pageNumber, extraKeys = [], preferredKey = null, quotaState = null) {
  ...
  // the same key×model fallback idea: primary model first, then a
  // fallback model, on every key, before giving up. Deliberately
  // HARDCODED (not read from env.*) so every image-description situation
  // in this bot — new book ingestion, /continue_book, and
  // /fix_legacy_images — always uses the exact same two models
  // regardless of what GEMINI_GENERATION_MODEL/_FALLBACK/_LITE are set to
  // in .env. This was previously a single key/single model call with no
  // fallback at all, which meant a batch of 100+ pages captioned back-to-
  // back burned through the free-tier's daily generateContent quota on
  // the primary model alone and every subsequent page failed with 429
  // RESOURCE_EXHAUSTED. Falling through to the second model spreads the
  // same batch across two separate per-model quotas instead of hammering
  // one. Deliberately no sleep/backoff/round-repeat here (unlike
  // generateJSON) — this stays a best-effort, non-blocking step from the
  // caller's point of view. It DOES now throw on total failure (see
  // below) instead of silently returning a placeholder caption, so the
  // caller can no longer mistake a failed page for a real success.
