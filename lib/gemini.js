...
async function generateJSON(prompt, triedKeysThisRound = new Set(), round = 1) {
  const key = pickKey(triedKeysThisRound);
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      ...
      if (newTried.size < env.GEMINI_API_KEYS.length) {
        return generateJSON(prompt, newTried, round);
      }
      ...
      if (round < MAX_ROUNDS) {
        ...
        const waitMs = suggested ?? Math.min(2 ** round * 1000, 30000);
        await sleep(waitMs + 500);
        return generateJSON(prompt, new Set(), round + 1);
      }
    }
