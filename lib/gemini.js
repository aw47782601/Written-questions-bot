...
async function notifyFallbackUsed(usedLabel, usedModel, attempts) {
  if (attempts.length === 0) return; // succeeded on the very first try — nothing to report
  await notifyAdmins(
...
async function runFallbackSequence(prompt, maxOutputTokens, responseSchema, extraKeys) {
  ...
  const keys = shuffled(extraKeys.length > 0 ? [...baseKeys, ...extraKeys] : baseKeys);
  const attempts = [];
...
async function generateJSON(prompt, { maxOutputTokens = 8192, responseSchema = null, extraKeys = [] } = {}, round = 1, priorAttempts = []) {
  ...
  if (!data) {
    ...
    if (retryable && round < MAX_ROUNDS) {
      ...
      const waitMs = suggested ?? Math.min(2 ** round * 1000, 30000);
      await sleep(waitMs + 500);
      return generateJSON(prompt, { maxOutputTokens, responseSchema, extraKeys }, round + 1, allAttempts);
    }
  ...
  if (candidate?.finishReason === 'MAX_TOKENS' && maxOutputTokens < 32768) {
    return generateJSON(prompt, { maxOutputTokens: maxOutputTokens * 2, responseSchema, extraKeys }, round, allAttempts);
  }

  if (!rawText) throw new Error('Gemini returned no content');

  await notifyFallbackUsed(usedKeyLabel, usedModel, allAttempts);
