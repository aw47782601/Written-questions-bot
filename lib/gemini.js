...
async function captionImage(buffer, mimeType, pageNumber, extraKeys = []) {
  ...
  // and no admin notification on failure — this stays a best-effort,
  // non-blocking step: a captioning failure falls back to a generic
  // placeholder caption rather than blocking the whole image-ingestion
  // pass or spamming admins once per page across a large batch.
  ...
  console.error(
    `captionImage failed for page ${pageNumber} after ${attempts.length} attempt(s) across ${keys.length} key(s) × ${models.length} model(s):`,
    last ? `${last.model} → ${last.status}: ${(last.errText || '').slice(0, 200)}` : 'no attempts were made'
  );
