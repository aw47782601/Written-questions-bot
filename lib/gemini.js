...
async function captionImage(buffer, mimeType, pageNumber, extraKeys = [], preferredKey = null) {
  const prompt =
  ...
  let keys = extraKeys.length > 0 ? [...shuffled(extraKeys), ...shuffled(baseKeys)] : shuffled(baseKeys);

  // If the caller pinned a specific key (books.js gives each concurrently
  // running worker its own key up front so N workers in flight at once
  // don't randomly pile onto the same key), try it first. Every other key
  // is still tried after it in shuffled order if it fails — retry/
  // fallback behavior across keys×models is otherwise unchanged.
...
module.exports = { embedTexts, embedOne, embedImages, captionImage, generateJSON, formatAttemptLog, shuffled };
