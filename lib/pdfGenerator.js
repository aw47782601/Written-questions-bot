...
// reordered per line via lib/textShaping.js before drawing, since PDFKit
// itself has no Arabic contextual-shaping or bidi support. Inline
// bold-highlighting is intentionally not attempted for Arabic paragraphs
// (accurately re-flowing per-word color spans through the bidi reorder
// step is a lot more work for little practical benefit here) — Arabic
// answers render as a single plain-colored right-aligned paragraph.
...
// { text, bold } word tokens (asterisks stripped).
...
// Greedy word-wrap for {text,bold} tokens, measuring each word with its
// own font (bold words are a little wider than regular ones).
...
function drawTokenLines(doc, lines, x, y, size, lineHeight) {
  ...
  lines.forEach((line, i) => {
    ...
    line.forEach((tok) => {
      ...
      doc.text(tok.text, cx, ly, { lineBreak: false });
      cx += doc.widthOfString(tok.text) + spaceWidth;
    });
  });
}
...
function drawFooter(doc, pageNum, pageWidth, pageHeight) {
  ...
  const label = shapeArabicLine(`صفحة ${pageNum}`);
  doc.text(label, 0, bandY + 8, { width: pageWidth, align: 'center' });
...
function generateAnswersPdf(results, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const title = meta.title || 'إجابات الأسئلة';
      ...
      results.forEach((r, idx) => {
        ...
        const pageNoteText = r.page ? `(صفحة ${r.page})` : null;
        ...
        if (pageNoteText) {
          ...
          const shaped = shapeArabicLine(pageNoteText);
          const w = doc.widthOfString(shaped);
          doc.text(shaped, contentX + contentWidth - CARD_PADDING - w, curY, { lineBreak: false });
        }
