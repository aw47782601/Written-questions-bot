...
// Splitting is only worth it if there's room left for the question +
// divider + ANSWER label + at least this many real answer lines — a
// sliver of 1 line isn't worth an extra rounded box + note, so anything
// smaller than this just pushes the whole card to the next page instead
// (same as before this feature existed).
const MIN_SPLIT_ANSWER_LINES = 2;
...
function generateAnswersPdf(allResults, meta = {}) {
  // This design no longer renders unanswered questions as trailing cards
  // (page === null or isError) — they're dropped from the PDF entirely.
  // The caller (see api/telegram-webhook.js deliverResults) is the one
  // that tells the user how many were left unanswered, in the delivery
  // caption, instead of the PDF carrying them.
  const results = allResults.filter((r) => !r.isError && r.page !== null);
