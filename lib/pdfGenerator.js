...
// spoiler (optional): when true, the answer is being rendered in "🙈 نص
// مشوش" mode — every word is drawn in the SAME color as the card
// background (colors.cream) instead of its normal color, so it's
// invisible at a glance but still real, selectable text: dragging/
// selecting it in a PDF viewer reveals it via the selection highlight,
// which is the closest a static PDF can get to Telegram's "press to
// reveal" spoiler. Only answer content is masked this way — the
// question, the "ANSWER:" label, and page citation stay fully visible.
function drawTokenLines(doc, colors, lines, x, y, size, lineHeight, spoiler = false) {
  lines.forEach((line, i) => {
    ...
    line.forEach((tok) => {
      doc.font(tokenFont(tok)).fontSize(size);
      const spaceWidth = doc.widthOfString(' ');
      doc.fillColor(spoiler ? colors.cream : tok.bold ? colors.orangeDark : colors.text);
...
function drawAnswer(doc, colors, measured, x, y, width, spoiler = false) {
  let cy = y;
  measured.blocks.forEach((b) => {
    ...
    if (b.arabic) {
      ...
      drawArabicLines(doc, b.lines, x, cy, innerWidth, FONT_ARABIC_REGULAR, ANSWER_SIZE, LINE_HEIGHT_A, spoiler ? colors.cream : colors.text);
      // The bullet marker itself is left visible (a masked dot the same
      // color as the card would be pointless) even in spoiler mode.
      if (b.bullet) doc.circle(x + width - 5, cy + 5, 2).fill(colors.orangeDark);
    } else {
      drawTokenLines(doc, colors, b.lines, b.bullet ? x + BULLET_INDENT : x, cy, ANSWER_SIZE, LINE_HEIGHT_A, spoiler);
...
function drawBodyCellText(doc, colors, cell, x, y, colWidth, spoiler = false) {
  const innerX = x + TABLE_CELL_PAD;
  const innerWidth = colWidth - TABLE_CELL_PAD * 2;
  if (cell.arabic) {
    drawArabicLines(doc, cell.lines, innerX, y + TABLE_CELL_PAD, innerWidth, FONT_ARABIC_REGULAR, TABLE_FONT_SIZE, TABLE_ROW_LINE_H, spoiler ? colors.cream : colors.text);
  } else {
    cell.lines.forEach((line, i) => {
      let cx = innerX;
...
function drawComparisonTable(doc, colors, measured, x, y, width, spoiler = false) {
  ...
  bodyRows.forEach((row, ridx) => {
    const rowBg = ridx % 2 === 0 ? colors.cream : colors.white;
    doc.rect(x, ry, width, row.height).fill(rowBg);
    let rx = x;
    row.cells.forEach((cell, i) => {
      // In spoiler mode, mask text to THIS row's own background (rows
      // alternate cream/white) rather than a fixed color, otherwise it'd
      // show up as a faint, readable color on the other row's background.
      drawBodyCellText(doc, { ...colors, cream: rowBg }, cell, rx, ry, colWidths[i], spoiler);
      rx += colWidths[i];
    });
    ry += row.height;
  });
...
function drawAnswerBody(doc, colors, body, x, y, width, spoiler = false) {
  let cy = y;
  if (body.kind === 'table') {
    if (body.intro) {
      drawAnswer(doc, colors, body.intro, x, cy, width, spoiler);
      cy += body.intro.height + PARA_GAP;
    }
    drawComparisonTable(doc, colors, body.measured, x, cy, width, spoiler);
  } else {
    drawAnswer(doc, colors, body.measured, x, y, width, spoiler);
  }
}
...
function generateAnswersPdf(results, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // 🙈 نص مشوش mode: answer text is drawn in the same color as its own
      // background (see drawTokenLines/drawArabicLines/drawBodyCellText
      // above) so it's invisible at a glance but still real, selectable
      // text — dragging/selecting over it reveals it via the PDF viewer's
      // selection highlight, the closest a static PDF gets to Telegram's
      // tap-to-reveal spoiler.
      const spoiler = !!meta.spoiler;
      ...
      results.forEach((r, idx) => {
        ...
        if (spoiler) {
          const labelW = doc.widthOfString('ANSWER:');
          const hint = shapeArabicLine('(اسحب فوق النص عشان تظهر الإجابة)');
          doc.font(FONT_ARABIC_REGULAR).fontSize(9).fillColor(colors.gray);
          doc.text(hint, contentX + CARD_PADDING + labelW + 8, curY + 1, { lineBreak: false });
        }
        curY += LABEL_SIZE + 6;

        // Answer body
        drawAnswerBody(doc, colors, aBody, contentX + CARD_PADDING, curY, contentWidth - CARD_PADDING * 2, spoiler);
        curY += aBody.height;
