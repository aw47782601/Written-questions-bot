...
// Comic Neue has zero Greek glyph coverage (confirmed via font cmap
// inspection), so any answer using math/science symbols like α β γ δ
// silently drew as blank/missing-glyph boxes. Lato covers essentially the
// whole Greek alphabet, so it's registered here purely as a per-word
// fallback — see tokenFont() below — without changing the design's normal
// Comic Neue look for everything else.
...
function drawChapterHeader(doc, colors, chapterText, x, y, width) {
  doc.roundedRect(x, y, width, CHAPTER_HEADER_H, 8).fill(colors.orange);
  ...
  if (isMostlyArabic(chapterText)) {
    ...
    doc.font(FONT_ARABIC_BOLD).fontSize(CHAPTER_HEADER_SIZE).fillColor(colors.white);
    const w = doc.widthOfString(shaped);
    doc.text(shaped, x + Math.max(0, (width - w) / 2), textY, { lineBreak: false });
  } else {
    doc.font(FONT_BOLD).fontSize(CHAPTER_HEADER_SIZE).fillColor(colors.white);
    doc.text(chapterText, x, textY, { width, align: 'center', lineBreak: false });
  }
}
