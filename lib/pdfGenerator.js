...
// edge), with a semi-transparent embossed "BY M.E.M" watermark floating
// in the white space directly above them — the watermark is NOT part of
// the checker rows themselves.
const FOOTER_CHECKER_H = CHECKER_SIZE * 2;
const FOOTER_WATERMARK_SIZE = 27;
const FOOTER_WATERMARK_GAP = 12; // space between watermark baseline area and the checker rows
const FOOTER_TOP_PAD = 10;
const FOOTER_H = FOOTER_TOP_PAD + FOOTER_WATERMARK_SIZE + FOOTER_WATERMARK_GAP + FOOTER_CHECKER_H;
...
function drawCheckerStrip(doc, colors, x, y, width, squareSize, rowIndex = 0) {
  const cols = Math.ceil(width / squareSize);
  doc.save();
  for (let i = 0; i < cols; i++) {
    doc.rect(x + i * squareSize, y, squareSize, squareSize).fill((i + rowIndex) % 2 === 0 ? colors.orange : colors.white);
  }
  doc.restore();
}
...
// Brand watermark shown centered above the footer's checkerboard rows,
// echoing the reference template's "BY M.E.M" signature: a bold display
// wordmark (same font as the rest of the M.E.M branding) rendered
// semi-transparent and outlined, with a subtle metallic silver/gray
// gradient fill, so it reads as an embossed watermark rather than solid
// text. Kept as a constant (rather than derived from meta.title) since
// it's a fixed brand tag, not per-document content; override via
// meta.footerBrand if a caller ever needs a different tag.
const DEFAULT_FOOTER_BRAND = 'BY M.E.M';
const FOOTER_WATERMARK_OPACITY = 0.4; // 35-45% opacity
const FOOTER_PAGE_NUM_SIZE = 12;

function drawFooter(doc, colors, pageNum, pageWidth, pageHeight, brand) {
  const checkerY = pageHeight - FOOTER_CHECKER_H;
  const watermarkY = checkerY - FOOTER_WATERMARK_GAP - FOOTER_WATERMARK_SIZE;

  // Watermark: semi-transparent, outlined, metallic gray gradient fill.
  const brandText = (brand || DEFAULT_FOOTER_BRAND).toUpperCase();
  doc.font(FONT_HEADING).fontSize(FOOTER_WATERMARK_SIZE);
  const textW = doc.widthOfString(brandText);
  const textX = (pageWidth - textW) / 2;

  const gradient = doc.linearGradient(textX, watermarkY, textX, watermarkY + FOOTER_WATERMARK_SIZE);
  gradient.stop(0, colors.white).stop(0.5, colors.gray).stop(1, colors.text);

  doc.save();
  doc.fillOpacity(FOOTER_WATERMARK_OPACITY).strokeOpacity(FOOTER_WATERMARK_OPACITY + 0.15);
  doc.fillColor(gradient).strokeColor(colors.orangeDark).lineWidth(0.75);
  doc.text(brandText, 0, watermarkY, {
    width: pageWidth,
    align: 'center',
  ...
  // Page number, bottom-right corner, plain bold dark-accent text, same
  // line as the watermark.
  doc.fillColor(colors.orangeDark).font(FONT_BOLD).fontSize(FOOTER_PAGE_NUM_SIZE);
  doc.text(String(pageNum), 0, watermarkY + (FOOTER_WATERMARK_SIZE - FOOTER_PAGE_NUM_SIZE) / 2, {
    width: pageWidth - PAGE_MARGIN,
    align: 'right',
  });

  // Exactly two plain checkerboard rows, flush with the bottom edge.
  drawCheckerStrip(doc, colors, 0, checkerY, pageWidth, CHECKER_SIZE, 0);
