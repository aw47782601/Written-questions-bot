...
- If a question ends with the literal tag "{table}" (with or without a space before it), KEEP that tag verbatim at the end of the question's wording — it is a formatting instruction the system uses later, NOT decoration/junk to remove.
...
// fallbackLog (optional): an array the caller provides to collect
// "extraction fell back to line-splitting for a chunk" events (each
// pushed as { model, error }) instead of this module notifying admins
// directly. The caller (api/telegram-webhook.js) folds these into the
// SAME single consolidated admin report used for the rest of the
// request (key/model used, failures, etc. — see buildUsageReportLines),
// rather than firing off a separate message here.
// generationLog (optional): an array the caller provides to collect
// "which key/model succeeded" for each AI extraction call (each pushed
// as { keyLabel, model }) — passed through to gemini.generateJSON as its
// `collector`, so it records the call instead of firing gemini.js's own
// standalone "🔑 تم استخدام ..." admin notification. The caller folds
// this into the same consolidated report as fallbackLog above, instead
// of it going out as its own separate admin message.
async function extractQuestionsFromText(text, fallbackLog = null, generationLog = null) {
  const chunks = splitIntoChunks(text);
  const items = [];
  let carryChapter = null;
  const collector = generationLog ? { generationCalls: generationLog } : null;
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      if (fallbackLog) {
        fallbackLog.push({ model: env.GEMINI_EXTRACTION_MODEL, error: err.message });
      }
...
async function extractQuestionsFromPdfBuffer(buffer, fallbackLog = null, generationLog = null) {
  const text = await extractFullText(buffer);
  return extractQuestionsFromText(text, fallbackLog, generationLog);
}

async function extractQuestionsFromPlainTextBuffer(buffer, fallbackLog = null, generationLog = null) {
  return extractQuestionsFromText(buffer.toString('utf-8'), fallbackLog, generationLog);
}
