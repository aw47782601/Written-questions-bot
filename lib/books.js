...
// Vision captioning (gemini.captionImage) already retries across every
// key × every fallback model before giving up (see gemini.js), the same
// resilience embedTexts has — so there's no reason to run it at a lower
// concurrency than the text-embedding path. Kept as one shared constant
// so captioning and embedding always move in lockstep instead of one
// silently lagging the other on large image-heavy books.
const MAX_CONCURRENT_IMAGE_CAPTIONS = MAX_CONCURRENT_EMBED_BATCHES;
...
async function ingestPageImages(bookId, pdfBuffer, onProgress, budgetMs, startTime, pages = null) {
  ...
  // Shuffled once per pass, then each concurrent worker is pinned to one
  // key of its own (worker 0 → captionKeyPool[0], worker 1 →
  // captionKeyPool[1], ...) instead of every captionImage() call picking
  // a random key independently. With MAX_CONCURRENT_IMAGE_CAPTIONS
  // workers running at once, random-per-call selection could easily send
  // two workers to the same key at the same moment, doubling up on that
  // key's rate limit for no reason; a fixed assignment guarantees zero
  // collisions whenever there are at least as many keys as workers, and
  // falls back to sharing keys round-robin (not randomly) if there are
  // fewer keys than workers. captionImage() still retries through every
  // OTHER key if a worker's own pinned key fails — this only changes
  // which key is tried first, not the fallback behavior.
  const captionKeyPool = gemini.shuffled(env.GEMINI_API_KEYS);
  await runWithConcurrency(
    images,
    MAX_CONCURRENT_IMAGE_CAPTIONS,
    async (img, idx, workerIndex) => {
      const preferredKey = captionKeyPool.length > 0 ? captionKeyPool[workerIndex % captionKeyPool.length] : null;
      captions[idx] = await gemini.captionImage(img.buffer, img.mimeType, img.page_number, [], preferredKey);
      captioned += 1;
      const now = Date.now();
  ...
  // Same fixed key-per-worker assignment as the captioning pass above —
  // shuffled once, then each concurrent batch worker keeps its own key
  // for the whole pass instead of picking one at random per batch.
  const imageEmbedKeyPool = gemini.shuffled(env.GEMINI_API_KEYS);
  const { timedOut } = await runWithConcurrency(
    ...
    async (batch, batchIdx, workerIndex) => {
      const preferredKey =
        imageEmbedKeyPool.length > 0 ? imageEmbedKeyPool[workerIndex % imageEmbedKeyPool.length] : null;
