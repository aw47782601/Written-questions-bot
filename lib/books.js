...
async function ingestPageImages(bookId, pdfBuffer, onProgress, budgetMs, startTime, pages = null, collector = null) {
  ...
  const { timedOut } = await runWithConcurrency(
    ...
    async (job, idx, workerIndex) => {
      ...
      // captionFailed tracks whether captionImage() itself threw. On
      // failure we still fall back to a generic placeholder string
      // ("تعذر توليد وصف تلقائي لها") so the pipeline has SOME text to
      // embed, but that placeholder is not a real caption. Previously, as
      // long as the embedding step for that placeholder succeeded, the row
      // was marked image_status: 'done' and counted in `done`/imageChunks
      // exactly like a genuine successful caption — so a page whose vision
      // call failed still showed up as a "success" in progress messages
      // and in books.total_chunks. Track the failure explicitly instead,
      // so it's never reported or counted as a success.
      let content;
      ...
      try {
        ...
        // — a placeholder-text row that merely embedded successfully is
        // NOT a success for this page, it still needs a retry (see
        // image_status: 'caption_failed', which a later backfill pass can
        // filter on to redo vision captioning specifically).
        if (!captionFailed) {
          done += 1;
        }
      } catch (err) {
        ...
        // scratch. If captioning ALSO failed, 'caption_failed' takes
        // priority so the backfill pass knows to redo captioning first,
        // not just embedding.
