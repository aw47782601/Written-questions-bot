...
async function storeSourcePdf(bookId, pdfBuffer) {
  ...
  try {
    ...
    //
    // "The object exceeded the maximum allowed size" is a SPECIFIC,
    // common failure: Supabase Storage enforces a project-wide GLOBAL
    // file size limit (Dashboard → Storage → Settings → "Global file
    // size limit") that takes precedence over any per-bucket
    // fileSizeLimit. The 200MB fileSizeLimit set on createBucket() above
    // only matters if it's LOWER than the global limit — it can never
    // raise the effective cap above it. On a fresh/free-tier project
    // that global limit commonly defaults to 50MB, so a large scanned
    // textbook PDF (hundreds of image-heavy pages) blows past it even
    // though the bucket itself claims to allow 200MB. Give a message
    // that actually points at the fix instead of a generic "re-upload"
    // instruction that will just fail again the same way.
    const isSizeLimitError = /exceeded the maximum allowed size|payload too large/i.test(err.message || '');
    const sizeMb = (pdfBuffer.length / (1024 * 1024)).toFixed(1);

    const message = isSizeLimitError
      ? `⚠️ فشل حفظ نسخة الـ PDF الأصلية للكتاب #${bookId} (حجم الملف ${sizeMb}MB) — تعدى حد الحجم العام ` +
        `لمشروع Supabase نفسه (مش حد الـ bucket). لازم تزوّد "Global file size limit" من ` +
        `Dashboard → Storage → Settings (وعلى الخطة المجانية غالبًا محدود بـ50MB ومش هينفع تزوده غير ` +
        `بترقية للخطة المدفوعة). الكتاب هيتجهز عادي من غير الجزء ده، بس /continue_book مش هيشتغل عليه ` +
        `لو اتقطعت المعالجة (هيحتاج رفع من جديد بدل ما يكمل).`
    ...
    gemini.notifyAdmins(message).catch(() => {});
  }
}
...
//
// budgetMs (optional): defaults to the full SOFT_TIME_BUDGET_MS, but
// continueAllUnfinishedBooks (below) passes in whatever's LEFT of one
// shared budget when resuming several books back-to-back in a single
// call, so the sweep as a whole still respects Vercel's 300s ceiling
// instead of giving every book its own fresh 260s and risking a hard
// kill mid-book.
async function continueBookImages(bookId, onProgress = () => {}, budgetMs = SOFT_TIME_BUDGET_MS) {
  ...
  const captionQuotaState = gemini.createCaptionQuotaState();
  // Same opt-in silencing switch as ingestNewBook/fixLegacyFailedImages
  // (see the comment above their usageCollector) — passing this into
  // processOneImageJob (which forwards it to gemini.embedTexts) makes
  // embedTexts record { keyLabel } into usageCollector.embeddingCalls
  // instead of firing an admin Telegram notification for every single
  // embedding call. Previously this was created but never actually
  // passed to processOneImageJob (null was passed instead), so a
  // /continue_book run on a book with hundreds of unfinished images sent
  // the admin one "🔑 تم استخدام..." message PER image — pure spam for a
  // job that already reports its own progress via onProgress.
  const usageCollector = { embeddingCalls: [], generationCalls: [], failures: [] };
  const keyPool = env.GEMINI_API_KEYS;
  const imageConcurrency = Math.max(
    1,
  ...
  const { timedOut } = await runWithConcurrency(
    ...
    async (job, idx, workerIndex) => {
      const preferredKey = imageKeyAssignments[idx];
      const success = await processOneImageJob(job, bookId, preferredKey, captionQuotaState, usageCollector);
      if (success) done += 1;
...
// /continue_book with NO id: instead of making the admin look up every
// half-finished book's id in /books one at a time, scan book_chunks for
// every DISTINCT book_id that still has image rows in 'pending',
// 'caption_failed', or 'caption_only' (the exact same filter
// continueBookImages itself uses) and resume each one in turn,
// automatically, from wherever it stopped.
//
// All the books found share ONE soft time budget (SOFT_TIME_BUDGET_MS)
// rather than each getting its own fresh 260s — otherwise a sweep across
// even 2-3 partially-done books could sail straight past Vercel's 300s
// maxDuration and get hard-killed instead of stopping cleanly. Whatever
// budget is left after each book is what's passed in as that book's
// budgetMs; if that runs out before every found book gets a turn, the
// leftover books are simply reported and left untouched (same rows,
// same statuses) — call /continue_book with no id again later and this
// picks up right where the sweep left off.
//
// A failure on ONE book (e.g. its source PDF was never saved so
// continueBookImages throws its "لازم ترفعه تاني" error) is caught and
// reported per-book, then the sweep moves on to the next book rather
// than aborting the whole run.
async function continueAllUnfinishedBooks(onProgress = () => {}) {
  if (!env.ENABLE_IMAGE_CHUNKS) return { skipped: true, books: [] };
  ...
  const { data: rows, error: fetchErr } = await supabase
    ...
    .eq('chunk_type', 'image')
    .in('image_status', ['pending', 'caption_failed', 'caption_only']);
  if (fetchErr) throw new Error(`Supabase error scanning for unfinished books: ${fetchErr.message}`);

  const bookIds = [...new Set((rows || []).map((r) => r.book_id))];

  if (bookIds.length === 0) {
    onProgress('✅ مفيش أي كتاب فيه صور لسه محتاجة إكمال دلوقتي — كله خلص.');
    return { skipped: false, books: [] };
  }

  onProgress(
    `🔎 لقيت ${bookIds.length} كتاب لسه فيه صور محتاجة إكمال: ${bookIds.map((id) => `#${id}`).join('، ')}.\n` +
      `بكمّلهم واحد ورا التاني...`
  );

  const results = [];
  for (const bookId of bookIds) {
    const remainingBudget = SOFT_TIME_BUDGET_MS - (Date.now() - startTime);
    if (remainingBudget < 15000) {
      onProgress(
        `⏳ الوقت المتاح للطلب ده خلص قبل ما نوصل لكتاب #${bookId} (وأي كتاب بعده). ` +
          `شغّل /continue_book تاني من غير ID عشان يكمل الباقي.`
      );
      results.push({ bookId, skippedForTime: true });
      break;
    }

    onProgress(`\n📘 كتاب #${bookId}:`);
    try {
      const result = await continueBookImages(bookId, onProgress, remainingBudget);
      results.push({ bookId, ...result });
    } catch (err) {
      console.error(`continueAllUnfinishedBooks failed for book ${bookId}:`, err.message);
      onProgress(`❌ فشل إكمال كتاب #${bookId}: ${err.message}`);
      results.push({ bookId, error: err.message });
    }
  }

  return { skipped: false, books: results };
}
