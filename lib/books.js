...
// fails (bucket missing, storage quota, etc.) the book still ingests
// normally — it just can't be resumed later via /continue_book if a
// Vercel timeout cuts the run short; the admin would have to re-upload
// it as a fresh book in that case, same as before this existed.
async function storeSourcePdf(bookId, pdfBuffer) {
  try {
    ...
    await setBookFields(bookId, { source_pdf_path: path });
  } catch (err) {
