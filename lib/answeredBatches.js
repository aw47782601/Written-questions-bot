...
- Use a generic aspect/detail layout instead of one-column-per-compared-item: "columns" should be exactly two strings — a label for the point/aspect name (e.g. "Aspect", "Type", "Feature" — whichever fits the student's own wording best) and a label for its detail (e.g. "Detail", "Description", "Explanation") — always in English, even when the student's own answer is in Arabic or any other language. "rows" should be one row per distinct point/type/example/characteristic the student's own answer contains, with the point's name/aspect bolded in the first cell and the student's own detail bolded in the second cell. Still never add or invent content the student didn't write.
