...
// 🔁✏️➕➖ Storage for the "retry failed / edit a question" follow-up
// buttons (see the big comment block above offerRetryReword in
// api/telegram-webhook.js). Mirrors the stage/peek pattern in
// lib/pendingBatches.js, but keyed separately since this holds the
// ALREADY-ANSWERED result set (plus its delivery settings) rather than
// the pre-answer staged questions — the two are staged at different
// points in the flow and must not collide/overwrite each other.
const ANSWERED_TTL_MS = 30 * 60 * 1000; // 30 minutes
...
async function peekBatch(userId, token) {
  ...
  if (Date.now() - (pending.createdAt || 0) > ANSWERED_TTL_MS) return null;
  return pending;
}
...
CRITICAL — even if wrong, DO NOT correct it: the student's answer may be factually incorrect, incomplete, contain mistakes, or even contradict the book. This does NOT matter and is NOT your concern. You are not a teacher grading or correcting this answer — you are only a typesetter laying out text that is already final. Never fix a wrong fact, never add a missing fact, never soften/hedge/qualify a claim, never insert a correction/note/caveat of any kind. Copy every claim exactly as the student wrote it, right or wrong, and only change how it is laid out (line breaks, bullets, bold markup, table structure).
