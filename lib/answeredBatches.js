...
// Arabic-script Unicode ranges (main block + supplement + presentation
// forms) — used to catch cases where Gemini ignores the "English only"
// instruction for forced-table column headers and writes them in Arabic
// (or any other non-Latin script) instead.
const ARABIC_CHAR_RE = /[\u0600-\u06FF\u0750-\u077F\uFB50-\uFDFF\uFE70-\uFEFF]/;

// Safety net for forceTable ({table} tag) items only — real comparison
// tables built from the student's own comparison answer are untouched and
// keep whatever language/labels fit their wording. If Gemini put Arabic
// (or generally non-Latin) text in the column headers despite the FORCED
// TABLE instruction asking for English column names, replace them with a
// fixed English fallback so the requirement holds regardless of model
// compliance. Row content/labels are left exactly as returned.
function sanitizeForceTableColumns(comparisonTable) {
  if (!comparisonTable || !Array.isArray(comparisonTable.columns)) return comparisonTable;
  const hasNonEnglish = comparisonTable.columns.some((c) => ARABIC_CHAR_RE.test(String(c || '')));
  if (!hasNonEnglish) return comparisonTable;
  const fallbackLabels = ['Aspect', 'Detail'];
  const columns = comparisonTable.columns.map((_, i) => fallbackLabels[i] || `Column ${i + 1}`);
...
function buildFormatPrompt(items) {
  // items: [{ question, rawAnswer, forceTable }]
...
FORCED TABLE ITEMS — very important: item number(s) ${forceTableNumbers.join(', ')} must ALSO be laid out as a TABLE, exactly like the COMPARISON ANSWERS rules above (same "isComparison"/"comparisonTable" shape, same asterisk highlighting), even though the question itself is NOT a comparison. For these items only:
- Use a generic aspect/detail layout instead of one-column-per-compared-item: "columns" should be exactly two strings — a label for the point/aspect name (e.g. "Aspect", "Type", "Feature" — whichever fits the student's own wording best) and a label for its detail (e.g. "Detail", "Description", "Explanation") — written in English ONLY, using Latin letters only, with NO Arabic words or Arabic script anywhere in either column header, regardless of what language the student's own answer is in. "rows" should be one row per distinct point/type/example/characteristic the student's own answer contains, with the point's name/aspect bolded in the first cell and the student's own detail bolded in the second cell. Still never add or invent content the student didn't write.
- This applies only to the listed item number(s) — every other item keeps following the COMPARISON ANSWERS rule (table only if the student's own text is actually a comparison) and the STRUCTURE rule otherwise.` : ''}
...
async function formatBatch(items, extraKeys = [], collector = null) {
  ...
  try {
    ...
    return result.map((r, i) => {
      ...
      const comparisonTable = isComparison
        ? (items[i].forceTable ? sanitizeForceTableColumns(r.comparisonTable) : r.comparisonTable) || null
        : null;
      return {
        question: items[i].question,
        isComparison,
        answer: r.answer || (isComparison ? '' : items[i].rawAnswer),
        comparisonTable,
        isError: false,
      };
    });
...
async function formatUserAnswers(items, extraKeys = [], collector = null) {
  const normalized = (items || [])
    .map((it) => ({
      ...
      rawAnswer: String(it?.rawAnswer || '').trim(),
