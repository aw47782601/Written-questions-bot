...
- Use a generic aspect/detail layout instead of one-column-per-compared-item: "columns" should be ["الجانب", "التفصيل"] (or whatever two-part label fits the student's own wording), and "rows" should be one row per distinct point/type/example/characteristic the student's own answer contains, with the point's name/aspect bolded in the first cell and the student's own detail bolded in the second cell. Still never add or invent content the student didn't write.
