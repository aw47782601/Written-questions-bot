...
// Read-only lookup, validating only the token (no TTL/expiry) — the
// buttons for a delivered batch stay usable indefinitely, since there's
// only ever ONE stored batch per user (this same key gets overwritten by
// saveBatch on their NEXT answered batch — see below), so "how long ago"
// doesn't matter on its own. Doesn't delete — a batch can be
// retried/reworded more than once (e.g. retry, then later reword a
// different question).
...
// createdAt — same token is kept so buttons shown on the (now-updated)
// redelivery message keep working.
