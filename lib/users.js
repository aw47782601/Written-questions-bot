...
// --- Per-user PDF color choice (/pdfcolor) --------------------------------
// Persists across sessions on users.pdf_color, same shape as
// selected_book_id above. A missing/unset value means "use the design's
// default color" — resolved against lib/pdfColors.js's DEFAULT_PDF_COLOR
// by the caller, not stored here, so the default can change later without
// a data migration. See migrations/add_pdf_settings.sql.

async function getUserPdfColor(userId) {
  const user = await getUserData(userId);
  return user ? user.pdf_color : null;
}

async function setUserPdfColor(userId, colorKey) {
  const { error } = await supabase.from('users').update({ pdf_color: colorKey }).eq('user_id', userId);
  if (error) console.error('setUserPdfColor error:', error.message);
}
