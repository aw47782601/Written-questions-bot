...
// NOTE: there used to be a persistent per-user PDF color preference here
// (users.pdf_color, set via a standing /pdfcolor command). That's gone —
// the user is now asked which color they want for THIS batch's PDF via
// inline buttons every time, same as the format choice above. See the
// ansclr_ callback and buildColorKeyboard in api/telegram-webhook.js. The
// users.pdf_color column itself is left alone (no migration needed) —
// it's just unused now.
