...
// extractionFailures (optional): "AI extraction fell back to plain
// line-splitting for a chunk" events collected in
// lib/questionExtractor.js at extraction time — carried along with the
// batch so it can still be folded into the ONE consolidated admin report
// sent once the user finally picks a format (see buildUsageReportLines in
// api/telegram-webhook.js), even though extraction happened well before
// that report is built.
// extractionGenerationCalls (optional): same idea, but for the
// successful "which key/model was used" side of AI extraction (see
// lib/questionExtractor.js's generationLog) — carried along so that too
// lands in the same single consolidated report instead of firing its own
// separate admin message at extraction time.
async function stageBatch(userId, { questions, bookId, extractionFailures = [], extractionGenerationCalls = [] }) {
