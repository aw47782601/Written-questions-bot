...
// CHILDOS_RANGES above is just "does the cmap have an entry for this
// codepoint" — it does NOT mean the glyph drawn for that codepoint is
// the actual character. This "free personal use" build watermarks its
// non-alphanumeric glyphs: 53 codepoints across the ranges above —
// every one of $ % + < = > @ ^ | ~ and most of the Latin-1 symbol block
// (¢ £ ¤ ¥ ¦ § © ª ¬ ® ° ± ² ³ µ ¶ ¹ º ¼ ½ ¾ × ß ÷), plus ƒ ˜ † ‡ ‰ ⁄ €
// ™ and the arrow block (← ↑ → ↓ ↔ ↕ ↖ ↗ ↘ ↙) and − — all point at one
// identical 912-operation outline (verified via fontTools: every other
// glyph in the font has a distinct, much shorter outline). That outline
// renders as a small repeating "personal use only" watermark pattern
// instead of the character, which is exactly what was showing up as a
// mangled glyph in place of "|" in the cover pill ("N Questions | M
// Chapters"). inChildos() must treat these as NOT covered so
// classifyWord() below routes them to the Lato fallback tier instead —
// Lato's cmap was checked and has real glyphs for all 53.
const CHILDOS_WATERMARKED = new Set([
  36, 37, 43, 60, 61, 62, 64, 94, 124, 126, 162, 163, 164, 165, 166, 167,
  169, 170, 172, 174, 176, 177, 178, 179, 181, 182, 185, 186, 188, 189,
  190, 215, 223, 247, 402, 732, 8224, 8225, 8240, 8260, 8364, 8482, 8592,
  8593, 8594, 8595, 8596, 8597, 8598, 8599, 8600, 8601, 8722,
]);

function inChildos(cp) {
  if (CHILDOS_WATERMARKED.has(cp)) return false;
...
// like "☆" sitting on its own), or a text token broken into runs of
// consecutive characters that share the same font tier.
//
// This is per-character, not per-word: a word like "a×b÷c" keeps "a",
// "b", "c" in Childos and only "×"/"÷" switch to Lato, instead of the
// old behavior of bumping the *entire* word to Lato the moment any one
// character needed it (which made "5+3=8" or "a×b" visibly change font
// mid-sentence for characters Childos actually has fine glyphs for).
function classifyWord(word) {
  if (VECTOR_GLYPHS[word]) return { text: word, vector: VECTOR_GLYPHS[word], segments: null };
  ...
  for (const ch of word) {
    ...
    if (VECTOR_GLYPHS[ch]) {
      outCh = ch === '☆' ? '•' : '◇'; // ☆→bullet, ♤/♧→open diamond (both Lato-covered stand-ins)
      tier = 'lato';
    } else if (LATO_GAP_CHARS.has(ch)) {
      tier = 'spectral';
    } else if (!inChildos(ch.codePointAt(0))) {
      tier = 'lato';
    }
    if (tier === curTier) {
      curText += outCh;
    } else {
      if (curText) segments.push({ text: curText, tier: curTier });
      curTier = tier;
      curText = outCh;
    }
  }
  if (curText) segments.push({ text: curText, tier: curTier });
  return { text: segments.map((s) => s.text).join(''), vector: null, segments };
}
...
function wordWidth(doc, word, isBold, size) {
  ...
  word.segments.forEach((seg) => {
    doc.font(fontForTier(seg.tier, isBold)).fontSize(size);
...
function drawText(doc, measured, x, y, width, font, arabicFont, size, lineHeight, color, align) {
  ...
  measured.lines.forEach((line, i) => {
    ...
    line.forEach((word) => {
      ...
      word.segments.forEach((seg) => {
        doc.font(fontForTier(seg.tier, isBold)).fontSize(size).fillColor(color);
        doc.text(seg.text, cx, ly, { lineBreak: false });
