...
// rendered bold). Used for the footer "BY M.E.M" wordmark (matching
// converter.py's own _stamp_checker_bands step) and now also for the big
// display headings (cover title, chapter banner) to give them a distinct,
// friendlier look than the Lato body text.
const FONT_HEADING_PATH = path.join(__dirname, '../assets/fonts/Fredoka-Bold-700.ttf');
const FONT_HEADING = 'Fredoka-Bold';

// Childos — a rounded, playful semibold face used for the small circular
// badges (question-number badge, option-letter markers) and the
// subchapter banner, so those little labels pop against the Lato body
// text without competing with the Fredoka display headings.
...
function measureText(doc, text, font, arabicFont, size, maxWidth) {
  ...
  return { arabic, lines: shapedLines };
}
...
function drawText(doc, measured, x, y, width, font, arabicFont, size, lineHeight, color, align) {
  ...
  const useAlign = align || (measured.arabic ? 'right' : 'left');
...
function measureCover(doc, meta, width) {
  ...
  const titleM = measureText(doc, meta.title, FONT_HEADING, FONT_ARABIC_BOLD, COVER_TITLE_SIZE, innerWidth);
...
function drawCover(doc, theme, meta, measured, x, y, width) {
  ...
  drawText(doc, measured.titleM, x + COVER_PAD_X, cy, width - COVER_PAD_X * 2, FONT_HEADING, FONT_ARABIC_BOLD, COVER_TITLE_SIZE, COVER_TITLE_LH, '#ffffff');
  ...
  doc.restore();
  doc.fillColor('#ffffff').font(FONT_BOLD).fontSize(10.5);
  doc.text(pillText, x + COVER_PAD_X + 13, cy + (COVER_PILL_H - 10.5) / 2 - 1, { lineBreak: false });
}
...
function measureChapterBanner(doc, chapter, width) {
  const titleM = measureText(doc, chapter.name, FONT_HEADING, FONT_ARABIC_BOLD, CHAPTER_TITLE_SIZE, width - CHAPTER_PAD_X * 2);
...
function drawChapterBanner(doc, theme, chapter, measured, x, y, width) {
  ...
  drawText(doc, measured.titleM, x + CHAPTER_PAD_X, y + CHAPTER_PAD_Y, width - CHAPTER_PAD_X * 2, FONT_HEADING, FONT_ARABIC_BOLD, CHAPTER_TITLE_SIZE, CHAPTER_TITLE_SIZE + 3, '#ffffff');
...
const CARD_PAD_LEFT = 30; // room for the badge overflowing the left edge
const BADGE_R = 14;
const QTEXT_SIZE = 13;
const QTEXT_LH = 17;
const VIP_SIZE = 8.5;
const VIP_PAD_X = 9;
const VIP_H = 16;
const OPT_MARKER_R = 10;
const OPT_MARKER_GAP = 8;
const OPT_TEXT_SIZE = 11.5;
const OPT_LH = 15;
const OPT_ROW_GAP = 6;
const FLAG_SIZE = 10;
const FLAG_LH = 13;
...
function drawCard(doc, theme, q, measured, x, y, width) {
  ...
  drawCircleBadge(doc, badgeCx, badgeCy, BADGE_R, theme.primary, String(q.number), '#ffffff', FONT_CHILDOS, 11.5, '#ffffff');
  ...
  measured.optsM.forEach((om, i) => {
    ...
    drawCircleBadge(doc, markerCx, markerCy, OPT_MARKER_R, theme.optBg, letter, theme.primaryDark, FONT_CHILDOS, 9.5);
...
function drawFooter(doc, theme, pageNum, pageWidth, pageHeight, brand) {
  ...
  doc.font(FONT_BOLD).fontSize(FOOTER_PAGE_NUM_SIZE).fillColor(theme.primaryDark);
...
function generateMcqPdf(questions, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Cover + how-to legend — once, at the very top of the bank.
      ...
      const howtoM = measureHowto(contentWidth);
      ensureSpace(howtoM.height);
      drawHowto(doc, theme, howtoM, contentX, y, contentWidth);
      y += howtoM.height + 18;
