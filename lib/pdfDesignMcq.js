...
// covers basic ASCII — any word containing a Greek letter (α β γ δ ...)
// or a symbol from the arrows/math-operator/misc-symbol ranges (→ ± × ÷
// ≤ ≥ √ ∞ ∑ ...) needs to fall back to Lato (FONT_GREEK_REGULAR/BOLD —
// see hasGreek() in lib/textShaping.js and the Fonts section above) or
// it silently draws as a blank/missing-glyph box. This is the same
// per-word fallback lib/pdfGenerator.js already uses for question/answer
// text; measureText/drawText here are the single shared helper for
// question text, option text, AND the answer-key table's explanation
// column, so fixing it here covers all three at once.
function measureText(doc, text, font, arabicFont, size, maxWidth) {
  ...
  // Non-Arabic: wrap word-by-word, tagging each word with whether it
  // needs the Greek/symbol fallback font, so drawText can switch fonts
  // mid-line without breaking the wrap width calculation.
  const greekFont = font === FONT_BOLD ? FONT_GREEK_BOLD : FONT_GREEK_REGULAR;
  const words = raw.split(/\s+/).filter(Boolean).map((w) => ({ text: w, greek: hasGreek(w) }));
  const lines = [];
  let current = [];
  let currentWidth = 0;
  const spaceWidth = doc.font(font).fontSize(size).widthOfString(' ');
  words.forEach((word) => {
    doc.font(word.greek ? greekFont : font).fontSize(size);
...
function lineWidth(doc, line, font, greekFont, size) {
  let width = 0;
  line.forEach((word, i) => {
    doc.font(word.greek ? greekFont : font).fontSize(size);
    if (i > 0) width += doc.widthOfString(' ');
    width += doc.widthOfString(word.text);
  });
  return width;
}
...
function drawText(doc, measured, x, y, width, font, arabicFont, size, lineHeight, color, align) {
  ...
  measured.lines.forEach((line, i) => {
    ...
    let cx = x;
    if (useAlign === 'center') {
      cx = x + Math.max(0, (width - lineWidth(doc, line, font, greekFont, size)) / 2);
    } else if (useAlign === 'right') {
      cx = x + Math.max(0, width - lineWidth(doc, line, font, greekFont, size));
    }
    line.forEach((word) => {
      doc.font(word.greek ? greekFont : font).fontSize(size).fillColor(color);
