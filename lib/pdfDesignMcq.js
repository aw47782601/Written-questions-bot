...
// Kept only for the footer "BY M.E.M" wordmark, matching converter.py's
// own _stamp_checker_bands step, which also stamps that brand in Fredoka.
...
// option-letter markers, and the small icon badges in the how-to legend.
function drawCircleBadge(doc, cx, cy, r, bg, text, textColor, font, size, ringColor) {
  ...
  if (text) {
    ...
    doc.text(text, cx - w / 2, cy - size / 2 + 1, { lineBreak: false });
  }
}
...
function drawCard(doc, theme, q, measured, x, y, width) {
  ...
  drawCircleBadge(doc, badgeCx, badgeCy, BADGE_R, theme.primary, String(q.number), '#ffffff', FONT_BOLD, 11.5, '#ffffff');
  ...
  measured.optsM.forEach((om, i) => {
    ...
    drawCircleBadge(doc, markerCx, markerCy, OPT_MARKER_R, theme.optBg, letter, theme.primaryDark, FONT_BOLD, 9.5);
