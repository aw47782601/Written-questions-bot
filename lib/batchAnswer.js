...
async function answerBatch(items) {
  ...
  try {
    const result = await gemini.generateJSON(prompt, { maxOutputTokens, responseSchema: BATCH_RESPONSE_SCHEMA });
    ...
    if (items.length === 1) {
      ...
      // instead of failing the whole batch. isError=true so the caller
      // knows NOT to cache this — it's a transient failure, not a real
      // answer, and caching it would permanently stick a "try again"
      // message to this question.
...
async function answerQuestions(questions) {
  ...
  if (toProcess.length > 0) {
    ...
    await usageTracker.checkAndIncrement(1);
    const embeddings = await gemini.embedTexts(toProcess.map((t) => t.question), 'RETRIEVAL_QUERY');
