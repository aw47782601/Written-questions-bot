...
- Use a generic aspect/detail layout instead of one-column-per-compared-item: "columns" should be ["الجانب", "التفصيل"] (or the book's own two-part label if it fits better), and "rows" should be one row per distinct point/type/example/characteristic that answers the question, with the point's name/aspect bolded in the first cell and the key detail bolded in the second cell.
