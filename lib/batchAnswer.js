...
// onError (optional): async (question, errMessage) => void — called with
// the *real* underlying error (e.g. the raw Gemini 429/503 body) whenever
// a single question ultimately fails after all retries/rotation, so the
// caller can push a live notification instead of relying on console.error,
// which just goes into Vercel's logs and nobody sees in real time.
async function answerBatch(items, extraKeys = [], onError = null) {
  ...
  try {
    ...
    if (items.length === 1) {
      ...
      if (onError) {
        try {
          await onError(items[0].question, err.message);
        } catch (notifyErr) {
          console.error('onError notify callback failed:', notifyErr.message);
        }
      }
      return [
        {
    ...
    const [left, right] = await Promise.all([
      answerBatch(items.slice(0, mid), extraKeys, onError),
      answerBatch(items.slice(mid), extraKeys, onError),
    ]);
    return [...left, ...right];
  }
}
...
async function answerQuestions(questions, bookId, extraKeys = [], onError = null) {
