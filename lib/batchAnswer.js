...
// Safety net for forceTable ({table} tag) questions only — real
// comparison tables are untouched and keep whatever language/labels fit
// the book. If Gemini put Arabic (or generally non-Latin) text in the
// column headers despite the FORCED TABLE instruction asking for English
// column names, replace them with a fixed English fallback so the
// requirement holds regardless of model compliance. Row content/labels
// are left exactly as returned.
...
- Use a generic aspect/detail layout instead of one-column-per-compared-item: "columns" should be exactly two strings — a label for the point/aspect name (e.g. "Aspect", "Type", "Feature" — whichever fits the question best) and a label for its detail (e.g. "Detail", "Description", "Explanation") — written in English ONLY, using Latin letters only, with NO Arabic words or Arabic script anywhere in either column header, regardless of what language the question, excerpts, or answer are in. "rows" should be one row per distinct point/type/example/characteristic that answers the question, with the point's name/aspect bolded in the first cell and the key detail bolded in the second cell.
