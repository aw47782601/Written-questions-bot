...
// Header: a thin solid accent rule at the very top, a title row, and a
// hairline underneath — no checkerboard.
const TOP_RULE_H = 6;
const HEADER_TITLE_H = 30;
const HEADER_H = TOP_RULE_H + HEADER_TITLE_H;
...
// Header: thin solid accent rule at the very top, then a small title
// row (meta.title, left-aligned) with a hairline underneath — no
// checkerboard, no icon.
function drawHeader(doc, colors, pageWidth, title) {
  doc.rect(0, 0, pageWidth, TOP_RULE_H).fill(colors.orange);
  const titleText = (title || 'ANSWER SHEET').toUpperCase();
  doc.font(FONT_HEADING).fontSize(13).fillColor(NEUTRAL_INK);
  doc.text(titleText, PAGE_MARGIN, TOP_RULE_H + 9, { characterSpacing: 1, lineBreak: false });
  doc.save();
  doc.strokeColor(NEUTRAL_BORDER).lineWidth(1);
...
function generateAnswersPdf(results, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      function startPage() {
        drawHeader(doc, colors, pageWidth, meta.title);
        y = PAGE_CONTENT_TOP;
      }
