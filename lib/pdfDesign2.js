...
//
// Used to be a fixed CHAPTER_HEADER_H (34) with lineBreak: false on both
// branches, so a title too long for one line just got cut off (English)
// or drawn past the right edge with no wrap check at all (Arabic).
// measureChapterHeader() now wraps the title onto as many lines as it
// needs and returns the resulting height; the bar and underline are then
// drawn at that height instead of the old constant.
const CHAPTER_HEADER_SIZE = 14;
const CHAPTER_HEADER_LINE_H = 18;
const CHAPTER_HEADER_PAD_TOP = 2;
const CHAPTER_HEADER_PAD_BOTTOM = 8; // gap between last text line and the underline
const CHAPTER_HEADER_GAP = 14;
const CHAPTER_BAR_W = 4;

function measureChapterHeader(doc, chapterText, pageContentWidth) {
  const textWidth = pageContentWidth - CHAPTER_BAR_W - 10;
  ...
  const lines = wrapWordsByWidth(doc, words, font, CHAPTER_HEADER_SIZE, textWidth, { characterSpacing: 0.5 });
  ...
  const height = CHAPTER_HEADER_PAD_TOP + textHeight + CHAPTER_HEADER_PAD_BOTTOM;
  return { arabic, lines: shapedLines, textWidth, height };
}

function drawChapterHeader(doc, colors, measured, x, y, width) {
  const { arabic, lines, textWidth, height } = measured;
  doc.rect(x, y, CHAPTER_BAR_W, height).fill(colors.orange);
  const textX = x + CHAPTER_BAR_W + 10;
  doc.font(arabic ? FONT_ARABIC_BOLD : FONT_HEADING).fontSize(CHAPTER_HEADER_SIZE).fillColor(NEUTRAL_INK);
  lines.forEach((line, i) => {
    const lineY = y + CHAPTER_HEADER_PAD_TOP + i * CHAPTER_HEADER_LINE_H;
    if (arabic) {
      const w = doc.widthOfString(line);
      doc.text(line, textX + Math.max(0, textWidth - w), lineY, { lineBreak: false, characterSpacing: 0.5 });
    } else {
      doc.text(line, textX, lineY, { width: textWidth, lineBreak: false, characterSpacing: 0.5 });
    }
  });
  doc.save();
...
function generateAnswersPdf(results, meta = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      function startPage() {
        // Explicit pure-white page background, so the light-gray card
        // fill above reliably reads as a distinct, non-white surface
        // rather than relying on PDFKit's implicit default.
        doc.rect(0, 0, pageWidth, pageHeight).fill(NEUTRAL_WHITE);
      ...
      results.forEach((r, idx) => {
        ...
        function drawFullCard() {
          ...
          const cardY = y;
          // Flat, slightly-tinted light-gray card (NOT pure white, and
          // deliberately distinct from the pure-white page background
          // this design draws on) with a thin neutral border, no
          // rounding — plus the colored left accent bar that replaces
          // design_1's filled cream background as the "this is one card"
          // visual cue.
          doc.rect(contentX, cardY, contentWidth, cardHeight).fillAndStroke(NEUTRAL_LIGHT, NEUTRAL_BORDER);
        ...
        if (cardHeight <= availableOnPage || aBody.kind !== 'text') {
          ...
          // 10, label + 6) — how much of a split card's first half is
          // taken up by the non-splittable header (question/divider/
          // label), before any answer-line space.
          const headerHeight = qMeasured.height + 8 + 10 + LABEL_SIZE + 6;
          ...
          if (firstBlocks.length === 0 || restBlocks.length === 0) {
            ...
            const cardY = y;
            // Same flat card + left accent bar as drawFullCard — no
            // special "open edge" treatment needed since this design
            // doesn't round its card corners to begin with.
            doc.rect(contentX, cardY, contentWidth, firstPartHeight).fillAndStroke(NEUTRAL_LIGHT, NEUTRAL_BORDER);
            doc.rect(contentX, cardY, CARD_ACCENT_BAR_W, firstPartHeight).fill(colors.orange);
            ...
            drawAnswer(doc, colors, { blocks: firstBlocks }, contentX + CARD_ACCENT_BAR_W + CARD_PADDING, curY, contentWidth - CARD_ACCENT_BAR_W - CARD_PADDING * 2);
            curY += firstAnswerHeight + 4;

            doc.font(FONT_REGULAR).fontSize(8.5).fillColor(NEUTRAL_GRAY);
            doc.text(`▾ Q${idx + 1} continues on next page…`, contentX + CARD_ACCENT_BAR_W + CARD_PADDING, curY, {
              width: contentWidth - CARD_ACCENT_BAR_W - CARD_PADDING * 2,
            });
            ...
            const cardY2 = y;
            doc.rect(contentX, cardY2, contentWidth, secondPartHeight).fillAndStroke(NEUTRAL_LIGHT, NEUTRAL_BORDER);
            doc.rect(contentX, cardY2, CARD_ACCENT_BAR_W, secondPartHeight).fill(colors.orange);

            let curY2 = cardY2 + CARD_PADDING;
            doc.font(FONT_REGULAR).fontSize(8.5).fillColor(NEUTRAL_GRAY);
            doc.text(`↳ Q${idx + 1} (continued):`, contentX + CARD_ACCENT_BAR_W + CARD_PADDING, curY2, {
              width: contentWidth - CARD_ACCENT_BAR_W - CARD_PADDING * 2,
            });
            curY2 += CONTINUATION_NOTE_H;

            drawAnswer(doc, colors, { blocks: restBlocks }, contentX + CARD_ACCENT_BAR_W + CARD_PADDING, curY2, contentWidth - CARD_ACCENT_BAR_W - CARD_PADDING * 2);
